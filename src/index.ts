...
async function solveOnPage(page: Page, log: (msg: string) => void, logError: (msg: string) => void): Promise<string> {
  ...
  const checkbox = await anchorFrame.waitForSelector('#recaptcha-anchor:not([disabled])', { timeout: 10000 })
  ...
  try {
    ...
    return page.evaluate(() => (document.getElementById('g-recaptcha-response') as HTMLInputElement).value)
  }
  ...
  while (true) {
    ...
    const audioLink = await imageFrame!.$eval('#audio-source', el => (el as HTMLSourceElement).src)
    ...
    try {
      await Promise.race([
        imageFrame!.waitForFunction(() => {
          const error = document.querySelector('.rc-audiochallenge-error-message') as HTMLElement | null
